...
- Verified against The Bumblezone (TelepathicGrunt/Bumblezone, `26.1` branch — a real, currently-maintained custom-dimension mod for this exact MC version) as a reference for real, working `noise_settings` JSON schema.
...
- Island tier shaping (small vs. rare "motherboard") not yet tuned — currently just reuses The End's natural island distribution as-is per the "even if simple/blocky" Phase 1 bar. Terrain island tops read as relatively flat (accurate to vanilla End's real look, not a bug) — adding an extra noise layer for more dramatic hills is a possible follow-up if wanted.
